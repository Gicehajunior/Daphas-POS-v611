          ...
          <div class="well">
            ...
            <strong>@lang('contact.total_purchase_due'): </strong><span class="display_currency" data-currency_symbol="true">{{ $contact_details->total_purchase - $contact_details->total_paid }}</span><br>
             @if(!empty($contact_details->opening_balance) || $contact_details->opening_balance != '0.00')
                  ...
                  {{ $ob_due }}</span>
              @endif
              @include('transaction_payment.partials.real-time-sale-due') 
          </div>
        </div>
        ...
        <div class="col-md-4">
          <div class="form-group">
            ...
            <div class="input-group">
              ...
              {!! Form::text("amount", @num_format($payment_line->amount), ['class' => 'form-control input_number payment_amount amount-to-pay', 'required', 'placeholder' => __('sale.amount'), 'data-rule-max-value' => $payment_line->amount, 'data-msg-max-value' => __('lang_v1.max_amount_to_be_paid_is', ['amount' => $amount_formated])]); !!}
              @else
                {!! Form::text("amount", @num_format($payment_line->amount), ['class' => 'form-control input_number payment_amount amount-to-pay', 'required', 'placeholder' => __('sale.amount')]); !!}
              @endif
            </div>
          </div>
        </div>
