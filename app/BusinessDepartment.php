...
class BusinessDepartment extends Model
{
    ...
    {
        $query = BusinessDepartment::where('business_id', $business_id)->Active();

        if ($check_permission) {
            $permitted_location_departments = auth()->user()->permitted_location_departments();
            if ($permitted_location_departments != 'all') {
                $query->whereIn('id', $permitted_location_departments);
            }
        }

        if ($append_id) {
            $query->select(
                DB::raw("IF(department_id IS NULL OR department_id='', department_name, CONCAT(department_name, ' (', department_id, ')')) AS department_name"),
                'id',
                'business_id',
                'location_id', 
            );
